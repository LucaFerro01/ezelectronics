...
describe('productExists', () => {
    ...
    test('should resolve to true if the product exists', async () => {
        const mockDBGet = jest.spyOn(db, "get").mockImplementationOnce((sql, params, callback) => {
            callback(null, { count: 1 });
            return {} as Database;
        });

        const result = await dao.productExists('existingModel');
    ...
    test('should resolve to false if the product does not exist', async () => {
        const mockDBGet = jest.spyOn(db, "get").mockImplementationOnce((sql, params, callback) => {
            callback(null, { count: 0 });
            return {} as Database;
        });

        const result = await dao.productExists('nonexistentModel');
    ...
    test('should reject with an error if the query fails', async () => {
        ...
        await expect(dao.productExists('model')).rejects.toThrow('Database error');
        expect(mockDBGet).toHaveBeenCalledTimes(1);
        expect(mockDBGet).toHaveBeenCalledWith(
            'SELECT COUNT(*) as count FROM Products WHERE model = ?',
            ['model'],
...
describe('reviewDone', () => {
    ...
    test('should resolve to true if the user has reviewed the product', async () => {
        const mockDBGet = jest.spyOn(db, "get").mockImplementationOnce((sql, params, callback) => {
            callback(null, { count: 1 });
            return {} as Database;
        });

        const result = await dao.reviewDone('model1', user);
        expect(result).toBe(true);
        expect(mockDBGet).toHaveBeenCalledTimes(1);
        expect(mockDBGet).toHaveBeenCalledWith(
            'SELECT COUNT(*) as count FROM Reviews WHERE model = ? AND userId = ?',
            ['model1', user],
            expect.any(Function)
        );
    });

    test('should resolve to false if the user has not reviewed the product', async () => {
        const mockDBGet = jest.spyOn(db, "get").mockImplementationOnce((sql, params, callback) => {
            callback(null, { count: 0 });
            return {} as Database;
        });

        const result = await dao.reviewDone('model2', user);
        expect(result).toBe(false);
        expect(mockDBGet).toHaveBeenCalledTimes(1);
        expect(mockDBGet).toHaveBeenCalledWith(
            'SELECT COUNT(*) as count FROM Reviews WHERE model = ? AND userId = ?',
    ...
    test('should reject with an error if the query fails', async () => {
        ...
        await expect(dao.reviewDone('model', user)).rejects.toThrow('Database error');
        expect(mockDBGet).toHaveBeenCalledTimes(1);
        expect(mockDBGet).toHaveBeenCalledWith(
            'SELECT COUNT(*) as count FROM Reviews WHERE model = ? AND userId = ?',
            ['model', user],
